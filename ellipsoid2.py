...
def compare():    
    ...
    def complexityE(n):
        return A0E*(4/(3*np.sqrt(3)))**n
    
    def complexityP(n):
        return A0P*(np.sqrt(5)/3)**n
    
    
    zP,areasP = polygon_algorithm(fd,vert)
    zE,areasE = ellipsoid2d(fd,G,E)
    print(zP)
    print(zE)
    
    xP=range(0,len(areasP))
    plt.plot(xP, areasP,'b')
    plt.plot(xP,list(map(complexityP,xP)),'r')
    
    plt.figure()    
    xE=range(0,len(areasE))
    plt.plot(xE,list(map(complexityE,xE)),'r')
    plt.plot(xE, areasE,'b')
    plt.show()    

def compareruntimes(startingpoint):    
    vert = [ #CCW felsorolt konvex polygon csucsai, minden csucs pontosan 1x, a szabalyokbol megkaphato lenne
            np.array([1,-1]),
            np.array([1,1]),
            np.array([-1,1]),
...
def visualcompare():    
    ...
    #X = np.array([np.linspace(-1,1,stepnum) for i in range(stepnum)])
    #Y = X.T
